...
void EnsurePathExists(const std::string& path, const char* description)
{
    if (path.empty()) {
        throw py::value_error(std::string(description) + " must not be empty");
    }

    if (!std::filesystem::exists(path)) {
        throw py::value_error(std::string(description) + " does not exist: " + path);
    }
}
...
public:
    ...
    {
        EnsurePathExists(vocabulary_path, "vocabulary_path");
        EnsurePathExists(settings_path, "settings_path");
        if (!load_atlas_path.empty()) {
            EnsurePathExists(load_atlas_path, "load_atlas_path");
        }
...
private:
    void EnsureSensor(
        std::initializer_list<ORB_SLAM3::System::eSensor> expected,
        const char* method_name
    ) const
    {
        for (const ORB_SLAM3::System::eSensor sensor : expected) {
            if (sensor_ == sensor) {
                return;
            }
        }

        throw py::value_error(std::string(method_name) + " is incompatible with the configured sensor");
    }
